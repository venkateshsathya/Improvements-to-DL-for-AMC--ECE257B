...
CFOmaxdev = 0  # 0 or 500.

model_file_list = [modelfilelocation + 'CNNoverRadioML_CFOmaxdev_' + str(CFOmaxdev)+'_L_1024__slowlearn_100EpochsRun1_model.pt']

snr_range = [-10, -6, -2, 0, 2, 6, 10, 15, 20]
acc_dict = {}
testdatatypes = ['Rician']
    ...
    for testdataidx in range(len(testdatatypes)):
        ...
        for snridx in range(len(snr_range)):
            ...
            for CFOidx in range(len(CFO_range)):
                ...
                # formulae is I==(Acos(theta) - Bsin(theta),   Q==(asin(theta) + bcos(theta))
                acostheta = np.multiply(data_temp2i, cos_array)
                bsintheta = np.multiply(data_temp2q, sin_array)
                asintheta = np.multiply(data_temp2i, sin_array)
                bcostheta = np.multiply(data_temp2q, cos_array)
                
                
                data_temp2i_corrected = acostheta - bsintheta
                data_temp2q_corrected = asintheta + bcostheta
